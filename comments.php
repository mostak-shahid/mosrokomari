...
<div id="comments">
	...
	<?php if (have_comments()) { ?>
		...
		<?php if (get_comment_pages_count() > 1 && get_option('page_comments')) { // are there comments to navigate through  ?> 
			<h3 class="screen-reader-text sr-only"><?php _e('Comment navigation', 'mosrokomaritemplate'); ?></h3>
			<ul id="comment-nav-above" class="comment-navigation pager" role="navigation">
				<li class="nav-previous previous"><?php previous_comments_link(__('&larr; Older Comments', 'mosrokomaritemplate')); ?></li>
				<li class="nav-next next"><?php next_comments_link(__('Newer Comments &rarr;', 'mosrokomaritemplate')); ?></li>
	...
	if (!comments_open() && '0' != get_comments_number() && post_type_supports(get_post_type(), 'comments')) { ?> 
		<p class="no-comments"><?php _e('Comments are closed.', 'mosrokomaritemplate'); ?></p>
	<?php 
	} //endif; 
	?> 
		...
		array(
			'fields' => array(
				'author' => '<div class="form-group">' .
							'<div class="col-md-4">' . 
							'<input id="author" name="author" type="text" value="' . esc_attr($commenter['comment_author']) . '" ' . $aria_req . ' class="form-control"  placeholder="' .  __('Name', 'mosrokomaritemplate') . '" />' . 
							'</div>',
				'email'  => '<div class="col-md-4">' . 
							'<input id="email" name="email" ' . ($html5 ? 'type="email"' : 'type="text"') . ' value="' . esc_attr($commenter['comment_author_email']) . '" ' . $aria_req . ' class="form-control"   placeholder="' .  __('Email', 'mosrokomaritemplate') . '" />' . 
							'</div>',
				'url'    => '<div class="col-md-4">' . 
							'<input id="url" name="url" ' . ($html5 ? 'type="url"' : 'type="text"') . ' value="' . esc_attr($commenter['comment_author_url']) . '" class="form-control"   placeholder="' .  __('Website', 'mosrokomaritemplate') . '" />' . 
							'</div>' . 
							'</div>',
			),
			'comment_field' => '<div class="form-group">' .  
							'<div class="col-md-12">' . 
							'<textarea id="comment" name="comment" rows="8" aria-required="true" class="form-control"  placeholder="' .  __('Comment', 'mosrokomaritemplate') . '"></textarea>' . 
							'</div>' . 
							'</div>',
			'comment_notes_after' => '<p class="help-block">' . 
							//sprintf(__('You may use these <abbr title="HyperText Markup Language">HTML</abbr> tags and attributes: %s', 'mosrokomaritemplate'), $comment_allowed_tags) . 
